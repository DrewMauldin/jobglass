...
| Linux         | per-user cron                     | Current user's direct spool file in supported `/var/spool/cron*` layouts when readable    | Definition evidence only                                          | Unreadable or absent spool files are explicit; JobGlass never invokes the privilege-bearing `crontab` helper | Fixtures and hosted build/test only |
| Linux         | system cron                       | `/etc/crontab`, `/etc/cron.d`, periodic directories when readable                         | Definition evidence only                                          | Unreadable files and directories are explicit                                                                | Fixtures and hosted build/test only |
