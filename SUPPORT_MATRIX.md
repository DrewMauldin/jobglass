...
| Linux         | per-user cron                     | Not read in v0.1.0                                                                        | Unavailable                                                       | JobGlass does not invoke the privilege-bearing `crontab` helper                                                                        | Parser fixtures only                |
| Linux         | system cron                       | `/etc/crontab`, `/etc/cron.d`, periodic directories when readable                         | Definition evidence only                                          | User crontabs are unavailable because JobGlass does not invoke the privilege-bearing helper; unreadable files/directories are explicit | Fixtures and hosted build/test only |
